...
public class MaxIncreaseKeepCitySkyline {
    public static void main(String[] args) {
        int[][] grid = new int[][]{
                ...
                {9, 2, 6, 3},
                {0, 3, 1, 0} };
        MaxIncreaseKeepCitySkyline maxIncreaseKeepCitySkyline = new MaxIncreaseKeepCitySkyline();
        int result = maxIncreaseKeepCitySkyline.maxIncreaseKeepingSkyline(grid);
        System.out.println(result);
    }
    public int maxIncreaseKeepingSkyline(int[][] grid) {
        int result = 0;
        int length = grid.length;
        int[] tSkyLines = new int[length];
        int[] lSkyLines = new int[length];

        for (int i = 0; i < length; i++) {
            for (int j = 0; j < length; j++) {
                if(grid[i][j] > tSkyLines[j]){
                    tSkyLines[j] = grid[i][j];
                }
                if(grid[i][j] > lSkyLines[i])
                    lSkyLines[i] = grid[i][j];
            }
        }

        for (int i = 0; i < length; i++) {
            for (int j = 0; j < length; j++) {
                result += Math.min(tSkyLines[j],lSkyLines[i]) - grid[i][j];
            }
        }
     ...
     * @return
     */
    public int maxIncreaseKeepingSkylineV2(int[][] grid) {
        int n = grid.length;
        int[] rh = new int[n], ch = new int[n];
        for (int i = 0; i < n; i ++) {
            for (int j = 0; j < n; j ++) {
                rh[i] = Math.max(rh[i], grid[i][j]);
                ch[j] = Math.max(ch[j], grid[i][j]);
            }
        }
        int inc = 0;
        for (int i = 0; i < n; i ++) {
            for (int j = 0; j < n; j ++) {
                inc += Math.min(rh[i], ch[j])-grid[i][j];
            }
        }
        return inc;
    }
