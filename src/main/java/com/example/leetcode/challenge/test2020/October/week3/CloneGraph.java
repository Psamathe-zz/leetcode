...
public class CloneGraph {
    ...
    public Node cloneGraph(Node node) {
        ...
        if(map.containsKey(node.val)){
            System.out.println("return created node:" + node.val);
            return map.get(node.val);
        }
        System.out.println("return new node:" + node.val);
        Node nodeClone = new Node(node.val);
        map.put(nodeClone.val,nodeClone);
        for (Node nodeNext : node.neighbors){
            nodeClone.neighbors.add(cloneGraph(nodeNext));
        }
        return nodeClone;
    }
