...
public class ReverseNodesInEvenLengthGroups {
    public static void main(String[] args) {
        ...
        ReverseNodesInEvenLengthGroups reverseNodesInEvenLengthGroups = new ReverseNodesInEvenLengthGroups();
        reverseNodesInEvenLengthGroups.reverseEvenLengthGroups(n3);
    }
    ...
    public ListNode reverseEvenLengthGroups(ListNode head) {
        ...
        while ( p != null){
            ...
            if(deque.size() == count){
                if(deque.size() % 2 == 0){
                    while (!deque.isEmpty()){
                        pRes.next = deque.pollLast();
                        pRes = pRes.next;
                    }
                } else {
                    while (!deque.isEmpty()){
                        pRes.next = deque.pollFirst();
                        pRes = pRes.next;
                    }
                }
                pRes.next = null;
