...
public class ThekStrongestValuesInArray {
    public static void main(String[] args) {
        ...
        int k = 3;
        ThekStrongestValuesInArray thekStrongestValuesInArray = new ThekStrongestValuesInArray();
        int[] result = thekStrongestValuesInArray.getStrongest(arr,k);
    ...
    public int[] getStrongest(int[] arr, int k) {
        ...
        int[] resize = Arrays.copyOf(arr,length);
        int medianIndex = (length - 1)/2;
        Arrays.sort(resize);
        int median = resize[medianIndex];

        arr = Arrays.stream(arr).boxed().sorted(new Comparator<Integer>() {
            @Override
